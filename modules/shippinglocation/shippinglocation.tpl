...
function store1(id_store)

{
	//alert('h');
	$('#stores').toggle('slow');
$('#current_store_name').html($('#store_name'+id_store).html());
$('#stores').css("visibility", "hidden");
$('#locationSelect').css("visibility", "hidden");
$('#stores').css("height", '10px');


//toggleMap();
$.post( "modules/shippinglocation/store.php", { store_id: id_store })

.done(function( data ) {

$('#d_store').html('Your default store is '+$("#store_name"+id_store).html()+'.<a href="#null" id="shopMapId" onclick="toggleMap();">Click here to change</a>');
...
<script type="text/javascript">
		...
		var infoWindow;
		var locationSelect;
		var default_store = '{$default_store_id}';
...
<h4>Your default store is <span id="current_store_name">{$store_data.name}</span> <a href="#null" id="shopMapId" onclick="showmap();">Click here to change </a></h4>

<div id="stores" style="visibility:hidden;height:10px;">
...
<p>{l s='Enter a location (e.g. zip/postal code, address, city or country) in order to find the nearest stores.'}</p>
	<p>
		...
		<input type="text" name="location" id="addressInput" value="{$pincode}" onclick="this.value='';" />
	</p>
	<p>
...
<input type="button" class="button" onclick="searchLocations();" value="{l s='Search'}" style="display: inline;" /> 
	...
	<div>
    <select id="locationSelect"><option></option></select></div>
    <div id="map"></div>
